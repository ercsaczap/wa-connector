# WaConnector - Typescript/Javascript WhatsApp Web API

WaConnector does not require Selenium or any other browser to be interface with WhatsApp Web, it does so directly using a **WebSocket**. Not running Selenium or Chromimum saves you like **half a gig** of ram :/
...
1. `cd path/to/WaConnector`
2. `yarn`
3. - `yarn example` for the multi-device edition
...
## Notable Differences Between WaConnector v3 & v4
...
type SocketConfig = {
  ...
  /** Should WaConnector ask the phone for full history, will be received async */
...
1. WaConnector, by default, emulates a chrome web session
...
WaConnector now fires the `connection.update` event to let you know something has updated in the connection. This data has the following structure:
...
WaConnector uses the EventEmitter syntax for events.
They're all nicely typed up, so you shouldn't have any issues with an Intellisense editor like VS Code.

The events are typed up in a type map, as mentioned here:

```ts
export type WaConnectorEventMap<T> = {
...
WaConnector does not come with a defacto storage for chats, contacts, or messages. However, a simple in-memory implementation has been provided. The store listens for chat updates, new messages, message updates, etc., to always have an up-to-date version of the data.
...
2. WaConnector has a function to generate the content for these link previews
...
In multi-device, you cannot mark an entire "chat" read as it were with WaConnector Web.
...
**Note:** In the multi-device version of WhatsApp -- if a desktop client is active, WA doesn't send push notifications to the device. If you would like to receive said notifications -- mark your WaConnector client offline using `sock.sendPresenceUpdate('unavailable')`
...
WaConnector is written with custom functionality in mind. Instead of forking the project & re-writing the internals, you can simply write your own extensions.
